...
| `probe_finds_an_empty_location_then_the_repository_once_created`, `deleting_a_rest_repository_is_refused_rather_than_attempted` (`tests/rest_server.rs`) | Against a real local `rustic-server`: probing tells empty from repository correctly by way of `config_id`; deleting one from Stellarshot is refused, and the repository is still there afterwards. **Caught along the way:** `rustic-server`'s own `--private-repos`/`RUSTIC_SERVER_PRIVATE_REPOS` default to on and do not actually turn off from the command line or environment despite accepting the flag, confirmed with its own `-v` debug log showing the override applied and then silently dropped when the config layers merge — worked around with a repository-specific ACL section instead of `[default]`. **Also caught:** waiting for the server's own "Listening on" log line, read from a piped stderr, hung indefinitely rather than timing out, because an abscissa app (what `rustic-server` is built on) fully buffers stderr once it is a pipe instead of a terminal; replaced with polling a real HTTP request against the port, which does not depend on that buffering. **Caught a third time, by CI rather than a local run:** both tests passed reliably on this project's own development machine but failed consistently in CI (three pushes in a row, confirmed by reading the actual CI logs rather than assumed from local results) with the same `Connect` error the round-trip test below was already known for. First tried retrying the whole scenario (a new server process, a new empty data directory, not `engine::init` in place, which is not idempotent) up to three times; every one of the three attempts failed identically in CI, which a genuine one-off race would not do, ruling that theory out. The actual cause: the Rust test harness runs every test function in this file concurrently by default, so both tests' own real `rustic-server` processes were competing for the CPU at once — never a problem on this development machine, but enough to make a constrained CI runner's servers too slow to answer within rustic_core's own retry window. Fixed with a process-wide `Mutex` (`ONLY_ONE_SERVER_AT_A_TIME`) held for a whole test's real work. Not reproduced locally without the fix (this machine has enough CPU that the contention never bites), but run with it under a tight CPU cap (`systemd-run -p CPUQuota=150%`, `--include-ignored`) as the closest local stand-in for a constrained CI runner: both pass every time; the round-trip test below still fails the same way even entirely alone under the same lock, which is exactly what tells apart the two causes — its own is not contention |
| `backup_and_restore_round_trip_through_a_rest_server` (`tests/rest_server.rs`) | Marked `#[ignore]`: passes by hand, but some of a backup's later requests intermittently fail against the same server with a connection error rather than an HTTP status, even once the server is confirmed actually serving requests. Confirmed **not** the same CPU-contention cause fixed above: still fails identically running entirely alone under `ONLY_ONE_SERVER_AT_A_TIME`, with nothing else competing for the CPU. Not yet root-caused; the other two tests above are unaffected |
