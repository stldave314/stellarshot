...
| Test | What it proves |
| --- | --- |
| `a_bandwidth_limit_is_passed_to_rclone`, `no_bandwidth_limit_adds_no_flag`, `a_local_location_ignores_a_bandwidth_limit` (`src/engine/repo.rs`) | A limit reaches the rclone command as `--bwlimit '<value>'`; none is added when empty; a local destination, which has no transfer to limit, ignores it |
