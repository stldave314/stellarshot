...
wizard-git-ignore = Honour .gitignore
...
wizard-append-only-description = A guard against mistakes, not attacks: rustic itself refuses to delete a snapshot from an append-only repository, but a tool that does not have to obey that could still remove one directly. Cannot be turned off later: Free up space, and the Keep setting above, stop working once this is on. Choose carefully; there is no way back from this screen.
